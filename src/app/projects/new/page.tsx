...
export default function NewProjectPage() {
  ...
  const [settings, setSettings] = useState<{ managers: { id: number; name: string }[]; object_types: { id: number; name: string }[] }>({ managers: [], object_types: DEFAULT_OBJECT_TYPES });
  ...
  useEffect(() => {
    fetch("/api/settings")
      ...
      .then((s) => setSettings({ managers: s.managers || [], object_types: s.object_types?.length ? s.object_types : DEFAULT_OBJECT_TYPES }))
      .catch(() => setSettings({ managers: [], object_types: DEFAULT_OBJECT_TYPES }));
  }, []);
